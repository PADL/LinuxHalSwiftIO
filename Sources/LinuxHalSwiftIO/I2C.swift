...
func swifthal_i2c_write(
  ...
  let i2c = arg.assumingMemoryBound(to: I2CHandle.self)
  if swifthal_i2c__set_slave(i2c.pointee.fd, UInt(address)) < 0 || write(
    i2c.pointee.fd,
    buf,
    length
  ) < 0 {
...
func swifthal_i2c_read(
  ...
  guard let arg, length >= 0 else { return -EINVAL }
  let i2c = arg.assumingMemoryBound(to: I2CHandle.self)
  if swifthal_i2c__set_slave(i2c.pointee.fd, UInt(address)) < 0 || read(
    i2c.pointee.fd,
    buf,
    length
  ) < 0 {
...
@c
func swifthal_i2c_dev_number_get() -> CInt {
  0
}
