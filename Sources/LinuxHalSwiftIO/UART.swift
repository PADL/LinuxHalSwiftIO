...
private func enableNbio(_ fd: CInt) -> CInt {
  ...
  if flags & O_NONBLOCK == 0 {
    if fcntl(fd, F_SETFL, flags | O_NONBLOCK) < 0 {
      swifthal__syslog(
        LOG_INFO,
        "LinuxHalSwiftIO: failed to enable non-blocking I/O on UART fd \(fd)"
      )
      return -errno
    }
  }
...
@c
func swifthal_uart_open(
  _ id: CInt,
  _ cfg: UnsafePointer<swift_uart_cfg_t>?
...
@c
func swifthal_uart_parity_set(
  _ arg: UnsafeMutableRawPointer?,
  _ parity: swift_uart_parity_t
) -> CInt {
  #if os(Linux)
  guard let arg else { return -EINVAL }
...
@c
func swifthal_uart_stop_bits_set(
  _ arg: UnsafeMutableRawPointer?,
  _ stopBits: swift_uart_stop_bits_t
) -> CInt {
  #if os(Linux)
  guard let arg else { return -EINVAL }
  let two: CInt
...
@c
func swifthal_uart_data_bits_set(
  _ arg: UnsafeRawPointer?,
  _ dataBits: swift_uart_data_bits_t
) -> CInt {
  #if os(Linux)
  guard let arg else { return -EINVAL }
  switch dataBits {
...
@c
func swifthal_uart_config_get(
  _ arg: UnsafeMutableRawPointer?,
  _ cfg: UnsafeMutablePointer<swift_uart_cfg_t>?
) -> CInt {
  #if os(Linux)
  ...
  cfg.pointee
    .parity = parity == 1 ? SWIFT_UART_PARITY_ODD :
    (parity == 2 ? SWIFT_UART_PARITY_EVEN : SWIFT_UART_PARITY_NONE)
...
@c
func swifthal_uart_char_get(
  _ arg: UnsafeMutableRawPointer?,
  _ c: UnsafeMutablePointer<UInt8>?,
  _ timeout: CInt
) -> CInt {
...
private func readBuffer(_ uart: UART, _ timeout: inout Int64?) -> Int {
  ...
  while uart.readBufOffset < uart.readBufLen {
    ...
    if pfd.revents & Int16(POLLIN) != 0 {
      let nbytes = read(
        uart.fd,
        uart.readBuf + uart.readBufOffset,
        uart.readBufLen - uart.readBufOffset
      )
...
@c
func swifthal_uart_dev_number_get() -> CInt {
  0
}
