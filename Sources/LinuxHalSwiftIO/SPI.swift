...
func swifthal_spi_open(
  ...
  _ speed: Int,
  _ operation: UInt16,
  _ wNotify: (@convention(c) (UnsafeMutableRawPointer?) -> Void)?,
  _ rNotify: (@convention(c) (UnsafeMutableRawPointer?) -> Void)?
...
@c
func swifthal_spi_config(_ arg: UnsafeMutableRawPointer?, _ speed: Int, _ operation: UInt16) -> CInt {
  #if os(Linux)
  guard let arg else { return -EINVAL }
  let spi = arg.assumingMemoryBound(to: SPIHandle.self)
  let fd = spi.pointee.fd
...
@c
func swifthal_spi_write(_ arg: UnsafeMutableRawPointer?, _ buf: UnsafePointer<UInt8>?, _ length: Int) -> CInt {
  guard let arg else { return -EINVAL }
  let spi = arg.assumingMemoryBound(to: SPIHandle.self)
...
@c
func swifthal_spi_read(_ arg: UnsafeMutableRawPointer?, _ buf: UnsafeMutablePointer<UInt8>?, _ length: Int) -> CInt {
  guard let arg else { return -EINVAL }
  let spi = arg.assumingMemoryBound(to: SPIHandle.self)
...
@c
func swifthal_spi_async_write(_ arg: UnsafeMutableRawPointer?, _ buf: UnsafePointer<UInt8>?, _ length: Int) -> CInt {
  -ENOSYS
}

@c
func swifthal_spi_async_read(_ arg: UnsafeMutableRawPointer?, _ buf: UnsafeMutablePointer<UInt8>?, _ length: Int) -> CInt {
  -ENOSYS
}
