...
@c
func swifthal_counter_read(_ arg: UnsafeMutableRawPointer?, _ ticks: UnsafeMutablePointer<UInt32>?) -> CInt {
  ticks?.pointee = 0
  guard let arg else { return -EINVAL }
  let c = arg.assumingMemoryBound(to: CounterHandle.self)
...
func swifthal_counter_add_callback(
  ...
  _ callback: (@convention(c) (UInt32, UnsafeRawPointer?) -> Void)?
) -> CInt {
  -ENOSYS
}
...
@c func swifthal_counter_dev_number_get() -> CInt { 0 }
