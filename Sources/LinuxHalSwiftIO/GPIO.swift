...
@c
func swifthal_gpio_interrupt_config(
  _ arg: UnsafeMutableRawPointer?,
  _ intMode: swift_gpio_int_mode_t
) -> CInt {
  #if os(Linux)
  guard let arg else { return -EINVAL }
  let gpio = handle(arg)
  ...
  switch intMode {
  case SWIFT_GPIO_INT_MODE_RISING_EDGE: lrc
    .request_type = CInt(GPIOD_LINE_REQUEST_EVENT_RISING_EDGE)
  case SWIFT_GPIO_INT_MODE_FALLING_EDGE: lrc
    .request_type = CInt(GPIOD_LINE_REQUEST_EVENT_FALLING_EDGE)
...
/// AsyncSwiftIO.
package func swifthal_gpio_interrupt_callback_install_block(
  _ arg: UnsafeMutableRawPointer,
  _ callback: @escaping (Bool, timespec) -> ()
) -> CInt {
  #if os(Linux)
  let gpio = handle(arg)
  ...
  source.setEventHandler { [weak gpio] in
    ...
    gpio.callback?(event.event_type == CInt(GPIOD_LINE_EVENT_RISING_EDGE), event.ts)
  }
  return 0
  #else
  return -ENOSYS
...
func swifthal_gpio_interrupt_callback_install(
  ...
  _ callback: (@convention(c) (UnsafeRawPointer?) -> ())?
) -> CInt {
  guard let arg else { return -EINVAL }
