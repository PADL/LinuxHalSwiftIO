...
@c
func swifthal_os_mq_send(_ mp: UnsafeRawPointer?, _ data: UnsafeRawPointer?, _ timeout: CInt) -> CInt {
  #if os(Linux)
...
@c
func swifthal_os_mq_recv(_ mp: UnsafeRawPointer?, _ data: UnsafeMutableRawPointer?, _ timeout: CInt) -> CInt {
  #if os(Linux)
  guard let mp, let data else { return -EINVAL }
  let p = mp.assumingMemoryBound(to: MQueue.self)
...
func swifthal_os_mq_purge(_ mp: UnsafeRawPointer?) -> CInt {
  ...
  while true {
    ...
    if mq_timedreceive(p.pointee.mq, data.assumingMemoryBound(to: CChar.self),
                       p.pointee.size, &prio, &ts) < 0 {
